...
The binary generated by "go build" will be named the same as the folder it is in, so be careful if you are using a CI server. For example, jenkins will use a folder named "workspace" by default; so you will need to rename this to "logstash-forwarder" or rename the output before packaging.
