...
func (p *Prospector) scan(path string, output chan *FileEvent, resumelist *ProspectorResume) {
  ...
  for _, file := range matches {
    ...
    if !is_known {
      ...
      } else if previous := is_file_renamed(file, fileinfo, p.prospectorinfo, missingfiles); previous != "" {
      ...
      if !is_fileinfo_same(lastinfo.fileinfo, fileinfo) {
        if previous := is_file_renamed(file, fileinfo, p.prospectorinfo, missingfiles); previous != "" {
...
func (p *Prospector) calculate_resume(file string, fileinfo os.FileInfo, resumelist *ProspectorResume) (int64, bool) {
  ...
  if previous := is_file_renamed_resumelist(file, fileinfo, resumelist.files); previous != "" {
